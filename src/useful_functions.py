...
def is_there_x_values(value_to_identify, col_list, dataset_0):
    ...
    for col in col_list:
        if (dataset_0[col] == value_to_identify).sum() != 0:
            print(f"There is a problem, there are {value_to_identify} in the column {col}. There are {(dataset_0[col] == value_to_identify).sum()} times the value {value_to_identify}")
